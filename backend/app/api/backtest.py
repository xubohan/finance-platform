...
from app.services.openbb_adapter import fetch_ohlcv, fetch_stock_snapshot, fetch_stock_universe_total
...
async def _upsert_ohlcv_rows(
    ...
    """Persist OHLCV rows for local fallback and reproducibility."""
...
async def _load_ohlcv_rows(
    ...
    result = await db.execute(
        stmt,
        {"symbol": symbol, "asset_type": asset_type, "start_date": start_date, "end_date": end_date},
    )
...
async def _fetch_ohlcv_with_local_fallback(
    db: AsyncSession | None,
    symbol: str,
    asset_type: str,
    start_date: str,
    ...
    """Prefer live fetch, then fallback to local DB cache if upstream fails."""
    ...
    if not latest_df.empty:
        ...
        return latest_df, {"source": "live", "stale": False}

    local_df = await _load_ohlcv_rows(db, symbol, asset_type, start_date, end_date)
    if local_df.empty:
        return pd.DataFrame(), {"source": "live", "stale": False}
    return local_df, {"source": "db", "stale": True}
...
async def run_backtest(payload: BacktestRequest, db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    ...
    df, ohlcv_meta = await _fetch_ohlcv_with_local_fallback(
        db=db_session,
        symbol=symbol,
    ...
    return {"data": result, "meta": {"ohlcv_source": ohlcv_meta["source"], "stale": ohlcv_meta["stale"]}}
...
async def run_backtest_lab(payload: BacktestLabRequest, db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    ...
    snapshot = fetch_stock_snapshot(market=payload.market, limit=payload.symbol_limit)
    if not snapshot:
        raise HTTPException(
            status_code=502,
            detail=_error(
    ...
    for item in snapshot:
        ...
        if df.empty:
            continue
        if ohlcv_meta.get("source") == "live":
            live_ohlcv_symbols += 1
        elif ohlcv_meta.get("source") == "db":
            local_fallback_symbols += 1
    ...
    return {
        ...
        "meta": {
            ...
            "total_available": fetch_stock_universe_total(payload.market),
            "ohlcv_live_symbols": live_ohlcv_symbols,
            "ohlcv_local_fallback_symbols": local_fallback_symbols,
        },
    }
