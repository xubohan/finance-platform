...
if run_compose ps >/dev/null 2>&1; then
  ...
  echo "[4/5] Run frontend tests/build locally"
  (cd frontend && npm test)
  (cd frontend && npm run build)

  echo "[5/5] Skip runtime smoke because Docker/stack access is unavailable in this shell"
fi
