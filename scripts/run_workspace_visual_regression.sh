...
echo "[1/4] Start core stack for live workspace capture"
run_compose up -d --build "${CORE_SERVICES[@]}"

echo "[2/4] Wait for market page"
wait_for_http "$BASE_URL/market" 30 2

echo "[3/4] Capture live workspace routes"
(cd frontend && node ./scripts/capture_workspace_visual_regression.mjs --base-url "$BASE_URL" --output-dir "$OUTPUT_DIR") | tee "$OUTPUT_DIR/run.log"

echo "[4/4] Live workspace capture artifacts saved to $OUTPUT_DIR"
